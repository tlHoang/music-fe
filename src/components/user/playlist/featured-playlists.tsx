  ...
  useEffect(() => {
    const fetchFeaturedPlaylists = async () => {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/playlists/featured`);
        ...
        if (result.success && result.data) {
          ...
          setError(result.message || "Failed to load featured playlists");
        }
      } catch (error) {
        console.error("Error fetching featured playlists:", error);
        setError("An error occurred while loading featured playlists");
      } finally {
        setLoading(false);
      }
    };
