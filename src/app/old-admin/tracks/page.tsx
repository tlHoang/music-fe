...
const TracksPage = () => {
  ...
  useEffect(() => {
    const fetchTracks = async () => {
      ...
      try {
        ...
        console.log("API Response:", responseData); // Debug the API response structure        // Handle different response structures
        let tracksArray = [];

        if (responseData && Array.isArray(responseData.data)) {
          // With our updated backend, tracks should be directly in data.data
          tracksArray = responseData.data.map((track: any) => ({
            ...track,
            coverImage: track.coverImage || track.cover,
          }));
          console.log("Using direct responseData.data array structure");
        } else if (
          responseData &&
          responseData.data &&
          ...
          tracksArray = responseData.data.data.map((track: any) => ({
            ...track,
            coverImage: track.coverImage || track.cover,
          }));
          console.log("Using nested responseData.data.data structure");
        } else if (responseData && Array.isArray(responseData)) {
          tracksArray = responseData.map((track: any) => ({
            ...track,
            coverImage: track.coverImage || track.cover,
          }));
  ...
  const getDefaultCoverImage = () => {
    return "/waveform.png"; // Use waveform image as default album art
  };
                  ...
                  <tr key={track._id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {track.userId ? (
                          ...
                          >
                            {track.userId.username || track.userId.name}
                          </Link>
                        ) : (
                          "Unknown Artist"
                        )}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      ...
                      <div className="text-xs text-gray-400">
                        {(track.likeCount || track.likes || 0).toLocaleString()}{" "}
                        likes
                      </div>
                      <div className="text-xs text-gray-400">
                        {(
